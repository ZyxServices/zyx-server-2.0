...
public class AccountCommonFacadeImpl implements AccountCommonFacade {
    ...
    public Map<String, Object> sendPhoneCode(String phone, String type, String message) {
        try {
            ...
            return MapUtils.buildErrorMap(AccountConstants.ACCOUNT_ERROR_CODE_40101, AccountConstants.ACCOUNT_ERROR_CODE_40101_MSG);
        } catch (Exception e) {
            e.printStackTrace();
    ...
    private Map<String, Object> doSendPhoneCode(String phone, String message) throws Exception {
        ...
        if (phone_code != null) {// 存在验证码
            return MapUtils.buildErrorMap(AccountConstants.ACCOUNT_ERROR_CODE_40007, AccountConstants.ACCOUNT_ERROR_CODE_40007_MSG);
        }
...
//            props.load(new InputStreamReader(RegisterFacadeImpl.class.getClassLoader().getResourceAsStream("SMS.properties"), CHARSET_UTF_8));
        props.load(new InputStreamReader(AccountRegisterFacadeImpl.class.getClassLoader().getResourceAsStream("CLMSG.properties"), CHARSET_UTF_8));
...
//            String substring = request.substring(beginPoint + 9, endPoint);
        ...
        if (returnString.contains(",0")) {
            ...
            if (returnString.contains(",103")) {
                ...
                return MapUtils.buildErrorMap(AccountConstants.ACCOUNT_ERROR_CODE_40014, AccountConstants.ACCOUNT_ERROR_CODE_40014_MSG);
            } else {
                return MapUtils.buildErrorMap(AccountConstants.ACCOUNT_ERROR_CODE_40009, AccountConstants.ACCOUNT_ERROR_CODE_40009_MSG);
            }
        }
    }
