...
import com.zyx.rpc.user.AccountLoginFacade;
 ...
 * @since 2016/11/8
 */
@Service("accountLoginFacade")
public class AccountLoginFacadeImpl implements AccountLoginFacade {
    ...
    public Map<String, Object> loginByPhoneAndPassword(String phone, String password) {
        try {
            ...
            if (count == 0) {// 未注册
                return MapUtils.buildErrorMap(AccountConstants.ACCOUNT_ERROR_CODE_40003, AccountConstants.ACCOUNT_ERROR_CODE_40003_MSG);
            }
            // 判断用户名密码是否正确
            AccountInfoVo accountInfo = accountLoginService.loginByPhoneAndPassword(new AccountLoginParam(phone, password));
